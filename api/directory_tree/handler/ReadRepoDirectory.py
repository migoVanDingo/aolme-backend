...
class ReadRepoDirectory(AbstractDirectoryTree):
    ...
    def do_process(self):
        ...
        try:
            current_app.logger.debug(f"{self.__class__.__name__} :: repo_id: {self.repo_id} :: entity_id: {self.entity_id}")    
            ...
            current_app.logger.debug(f"{self.__class__.__name__} :: all_dir_path: {all_dir_path}")
            file_list = []
            for dir in dir_list:
            ...
            current_app.logger.debug(f"{self.__class__.__name__} :: Response: {file_list}")
