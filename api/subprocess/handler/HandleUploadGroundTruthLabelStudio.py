...
class HandleUploadGroundTruthLabelStudio:
    ...
    def do_process(self, project_id, entity_id, dataset_id, subset_id):
        try:
            ...
            for f in files:
                f = "{}/{}".format(path, f)
                current_app.logger.debug(f"{self.__class__.__name__} :: files: {f}")
                arr.append(f)


            for file_name in arr:
                current_app.logger.debug(f"{self.__class__.__name__} :: file_name: {file_name}")
                ...
                try:
                    ...
                    if response.status_code == 201:
                        current_app.logger.debug(f"{self.__class__.__name__} :: Successfully imported {file_name} to project {project_id}")
            ...
            current_app.logger.debug(f"{self.__class__.__name__} :: Successfully imported all files to project {project_id}")
            return "success"
    ...
    def execute_curl_for_file_names(self, file_names, project_id, project_token=os.environ['LABEL_STUDIO_SECRET_KEY'], api_ur='http://localhost:8080'):
            current_app.logger.debug(f"{self.__class__.__name__} :: file_names: {file_names} :: project_id: {project_id}")
